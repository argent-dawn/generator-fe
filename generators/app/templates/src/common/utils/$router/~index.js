...
import <%= mainModuleName %>Conf from './modules/<%= mainModuleName %>';
...
const router = new VueRouter({
    routes: [{
        ...
        component(resolve) {
            require.ensure([], () => {
                resolve(require('@src/<%= mainModuleName %>/views/$app/index.vue'));
            }, 'views/<%= mainModuleName %>/$app/index');
        },
        meta: {
            keepAlive: false,   // 判断 页面是否需要 keep-alive 缓存
            rank: 20            // 动态判断 已缓存的页面 是否需要销毁缓存
        },
        children: <%= mainModuleName %>Conf
    }]
});
